...
export default function AvatarInput({
    ...
    accept = 'image/*',
    recommendation = 'square image, at least 400x400px',
}: Props) {
    ...
    return (
        <div>
            ...
            <div
                ...
                className={`group relative w-full cursor-pointer overflow-hidden rounded-full border border-dashed border-muted-foreground/25 bg-muted/50`}
                aria-label="Upload avatar image"
            >
                <div className={`w-full ${aspectClass} relative`}>
                    {/* Image preview or placeholder */}
                    {preview ? (
                        <img src={preview} alt="Avatar preview" className={'h-full w-full object-cover'} draggable={false} />
                    ) : (
                        <div className="flex h-full w-full items-center justify-center gap-4 p-6 text-center">
                            <div className="flex flex-col items-center">
                                ...
                                <p className="mt-3 text-sm text-muted-foreground">Upload an avatar image</p>
                                <p className="text-xs text-muted-foreground/80">Recommended: {recommendation}</p>
                            </div>
                        </div>
                    )}
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <Button type="button" onClick={removeImage} variant="destructive" size="sm" aria-label="Remove image">
